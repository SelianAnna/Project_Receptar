...
class RecipeCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # Získa zoznam ingrediencií z POST dát (arrays)
        names = self.request.POST.getlist('ingredient_name')
        amounts = self.request.POST.getlist('ingredient_amount')
        for name, amount in zip(names, amounts):
            if name.strip():
                ing, _ = Ingredient.objects.get_or_create(name=name.strip())
                RecipeIngredient.objects.create(recipe=self.object, ingredient=ing, amount=amount.strip())
